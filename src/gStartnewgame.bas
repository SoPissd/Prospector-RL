...
function gotslotforsavegame() As integer 
    Dim As String aText
...
function start_new_game(iAction as integer) As integer
	...
	If gotslotforsavegame()=0 Then return -1
    ...
    If configflag(con_startrandom)=1 Then
        b=textMenu("Choose ship/Scout/Long Range Fighter/Light Transport/Troop Transport/Pirate Cruiser/Random",text)
    Else
        b=rnd_range(1,4)
    EndIf
    player=make_ship(1)
...
'        next
    ...
    set__color( 11,0)
	'
	initialize_faction_standing(b=5)	
    If b<5 Then
...
function play_game(iAction as integer) as integer
    if iAction=7 then
		...
		player.dead=0
    	return 0	    	
    elseif (iAction=1 Or iAction=2) Andalso (player.dead=0) Then
        tVersion.gamerunning=1
        if (pDisplaystars<>null) andalso (pDisplayship<>null) andalso (pExplorespace<>null) then
	        pDisplaystars(1)
	        pDisplayship(1)
	        pExplorespace()
        EndIf
    EndIf
    If (player.dead>0) andalso (not (configflag(con_restart)=1 or player.dead=99)) Then
		death_message()
    EndIf
	player.dead=0
    'set__color(15,0)
